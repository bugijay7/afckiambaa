...
export default function EventList() {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        setEvents(data || []); // backend returns array directly
      } catch (err) {
  ...
  if (loading) return <div className="flex justify-center items-center min-h-screen text-gray-500">Loading events...</div>;
  ...
  return (
    <div className="min-h-screen bg-[#f8f5f0] pt-30 py-16 px-4 md:px-20 font-montserrat">
      ...
      <div className="flex justify-between items-center max-w-6xl mx-auto mb-10">
        <h1 className="text-lg uppercase tracking-wide text-gray-500">Upcoming Events</h1>
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        {events.length > 0 ? (
          events.map((event) => {
            ...
            return (
              <div key={event._id} className="flex flex-col md:flex-row text-white- rounded-xl shadow-md hover:shadow-lg overflow-hidden border border-gray-200 transition-all duration-300">
                {/* Image */}
                ...
                <div className="flex-1 p-6 text-left">
                  <h2 className="text-lg md:text-xl font-semibold text-gray-800 uppercase mb-2">{event.title}</h2>
